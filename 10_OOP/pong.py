...
class Bold:
    ...
    def update(self):
        ...
        if (
            ...
            or self.position_y > SKAERMHOEJDE - self.radius
        ):
            self.hastighed_y *= -1
...
class Bat:
    ...
    def __init__(
        self, center_x, center_y, hastighed_x, hastighed_y, bredde, hoejde, farve
    ):
    ...
    def draw(self):
        arcade.draw_rectangle_filled(
            self.center_x, self.center_y, self.bredde, self.hoejde, self.farve
        )
...
class Vindue(arcade.Window):
    ...
    def setup(self):
        ...
        self.spiller_venstre = Bat(
            20, SKAERMHOEJDE / 2, 0, 0, 10, 60, arcade.csscolor.WHITE
        )
        self.spiller_hoejre = Bat(
            SKAERMBREDDE - 20, SKAERMHOEJDE / 2, 0, 0, 10, 60, arcade.csscolor.WHITE
        )
        self.spiller_venstre_point = 0
        self.spiller_hoejre_point = 0
        self.spiller_venstre_pointtavle = arcade.Text(
            "0", 20, SKAERMHOEJDE - 20, font_size=20, anchor_x="left", anchor_y="top"
        )
        self.spiller_hoejre_pointtavle = arcade.Text(
            "0",
            SKAERMBREDDE - 20,
            SKAERMHOEJDE - 20,
            font_size=20,
    ...
    def update(self, delta_tid):
        ...
        if (
            self.bold.position_y
            < self.spiller_venstre.center_y + self.spiller_venstre.hoejde / 2
            and self.bold.position_y
            > self.spiller_venstre.center_y - self.spiller_venstre.hoejde / 2
            and self.bold.position_x - self.bold.radius
            < self.spiller_venstre.center_x + self.spiller_venstre.bredde / 2
        ):
        ...
        if (
            self.bold.position_y
            < self.spiller_hoejre.center_y + self.spiller_hoejre.hoejde / 2
            and self.bold.position_y
            > self.spiller_hoejre.center_y - self.spiller_hoejre.hoejde / 2
            and self.bold.position_x + self.bold.radius
            > self.spiller_hoejre.center_x - self.spiller_hoejre.bredde / 2
        ):
            self.bold.hastighed_x *= -1.1
